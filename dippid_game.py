...
#Position of our most crucial circle, the starting circle, you have to return to it every few seconds
anchor_x_pos = 500
anchor_y_pos = 400

#Positions of the other circles are stored in a list which is easier to access later instead of e.g. a tuple
left = [300,400]
right = [700,400]
up = [500,200]
down = [500,600]
...
# We do not load an extra PyQt5 UI and create everything ourself within the script
# Easier to run from terminal or other devices since you do not need an extra .ui file

    def initUIComponents(self):
        self.label1 = QtWidgets.QLabel(self)
        self.label1.setText("Welcome to the best reaction game ever! It's like a workout for your wrist! :-)")
...
# Always move the circle back to the beginning to guarantee a proper starting point for the accelerometer data
    def move_to_anchor(self):
            self.circle.move(anchor_x_pos, anchor_y_pos)

# With gravity being 9.81ms² we argue that 5.0 or above is an appropriate value for a clear signal to spin the phone correctly
...
# Do it again for all four possible target positions. Could also use a switch case or something else but it works fine
...
# We read Y-Data for the anchor since you need to hold the pone steady in front of you (z and x being irrelevant)
